...
#include "character/daveb.h"
#include "character/davec.h"
#include "character/daved.h"
#include "character/bambi.h"
#include "character/bambib.h"
#include "character/bambic.h"
#include "character/bambid.h"
#include "character/cheat.h"
#include "character/split.h"
...
static void Stage_LoadChart(void)
{
	...
	for (Note *note = stage.notes; note->pos != 0xFFFF; note++)
	{
		...
		if (note->type & NOTE_FLAG_OPPONENT)
			stage.player_state[1].max_score += 70;
		else
			stage.player_state[0].max_score += 70;
	}
				...
				case StageMode_Swap:
				{
					...
					for (Note *note = stage.cur_note;; note++)
					{
						...
						if (playing && (note->type & NOTE_FLAG_OPPONENT) && !(note->type & NOTE_FLAG_HIT))
						{
							...
							if (stage.stage_id >= StageId_5_1 && stage.stage_id <= StageId_5_2)
